...
if __name__ == '__main__':
    ...
    task_name = 'Task002_KidneyRegions'
    ...
    # labels
    if randomized:
        labels_dirs = [join(base, 'reviewed_masks_jr/regions'), join(base, 'reviewed_masks_vl/regions')]
    else:
        labels_dir = join(base, 'reviewed_masks_vl/regions')
    with open("split_80.json", "r") as fp:
        all_cases = json.load(fp)
    training_cases = all_cases["train"]
    testing_cases = all_cases["test"]

    training_selection = np.load("training_selection.npy")
    testing_selection = np.load("testing_selection.npy")
